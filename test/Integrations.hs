...
integrationSpec = describe "Docker Integration Tests" $ do
  around (withContainers setupContainers) $ do
    ...
    it "succeeds when a '--directive' successfully overrides a slurm submission error" $ \(slurm, _pbs) -> do
      let port = show $ D.containerPort slurm 22
    ...
    it "succeeds when a '--directive' overrides a pbs job submission error" $ \(_slurm, pbs) -> do
      let port = show $ D.containerPort pbs 22
